...
- **双外观（桌面态 / 前台态）**：可另设一套「有程序在前台时」的外观（位置、层级、穿透、不透明度、缩放），按前台窗口自动切换；判定规则：前台是可见且未最小化的**非桌面/外壳**窗口（不含 `Progman`/`WorkerW`/任务栏等）即视为「有程序在前台」（本应用主窗口也算），浮窗自身在前台不算；默认关闭，关闭时任何时刻都用桌面态外观；前台态默认**置顶显示**，避免被程序窗口完全挡住
